...
- **`validateAsync` exceptions on non-debounced change paths now route to `config.onError`** (issue #2). Pre-1.9.3, a throwing `validateAsync` (or throwing sync `validate`) on the non-debounced fire-and-forget paths — `setFieldValue` / `setFieldTouched` / `setTouched` when `validateDebounceMs == null` — propagated out of the unconfined `coroutineScope.launch { ... }` block in `scheduleChangeValidation` / `setFieldTouched` / `setTouched`, hitting the default coroutine exception handler. On Kotlin/Native this terminated the process with SIGABRT; on JVM it surfaced via the default uncaught-exception handler. The debounced collector path (v1.9.0) and `ComposeFormik.launch { }` (v1.9.1) already routed throws through `config.onError` — the non-debounced paths were the last asymmetric hole. A new private `runAllValidationsAndCommitRoutingErrors` helper in `FormikController.kt` mirrors the debounced collector's catch byte-for-byte: `CancellationException` propagates so structured cancellation keeps working, every other `Throwable` routes to `config.onError` when set (silently swallowed otherwise), and `FormikErrors.Empty` is committed. `scheduleChangeValidation` (non-debounced branch), `setFieldTouched`, and `setTouched` swap to the new helper; `applyArrayMutation` is transitively covered because it routes through `scheduleChangeValidation`. `submit()`, `validateForm()`, and `validateField()` continue to call the throwing `runAllValidationsAndCommit` directly — the **Formik #1329 contract (validator exceptions on explicit-trigger paths bubble to the caller) is preserved**, pinned by the existing `FormikIssueRegressionTest`, `ConcurrencyLifecycleTest`, and `CoverageGapsTest` validate-throws-rethrows tests, all green. The helper is `private`; `apiCheck` clean; no public-API change.

### Tests

- **5 load-bearing regression tests for issue #2 in `EdgeCasesTest` (`:kformik` commonTest)** pinning the no-debounce routing on each call site: `setFieldValue_throwingValidate_routes_to_onError_no_debounce`, `setFieldValue_throwingValidateAsync_routes_to_onError_no_debounce`, `setFieldTouched_throwingValidator_routes_to_onError_no_debounce`, `setTouched_throwingValidator_routes_to_onError_no_debounce`, `applyArrayMutation_throwingValidator_routes_to_onError_no_debounce`. Each one was verified load-bearing by reverting the helper swap on the corresponding call site and confirming the named test fails (audit-induction).
- **2 sibling tests in `EdgeCasesUiTest` (`:kformik-forms` jvmTest)** pinning the no-debounce path through the Compose adapter: `asyncValidator_throwing_routes_to_onError_noDebounce` and `blur_throwingValidateAsync_routes_to_onError_noDebounce`. The pre-existing `asyncValidator_throwing_routes_to_onError` test (which kept its `validateDebounceMs = 30` configuration) has its KDoc updated since the no-debounce asymmetry is now gone — both paths route identically.
- **Typed-V end-to-end test for KSP-generated updaters at `kformik-ksp/src/test/kotlin/io/kformik/ksp/TypedValuesEndToEndTest.kt`**: `typedDataClass_withKspGeneratedUpdater_roundTripsThrough_FormikController` exercises a `@FormValues`-annotated `data class` plus its KSP-generated `<Name>Updater : ValuesUpdater<Name>` wired into a `FormikController<MyData>` end-to-end. `setFieldValue` / `submit` / `resetForm` round-trip the typed values through the generated `copy()`-based `setAt`. Pins the headline value proposition of `:kformik-ksp` ("your form is `MyData`, not `Map<String, Any?>`"), which previously had only generation-correctness coverage and isolated-updater unit tests with no integration through the controller. `kformik-ksp/build.gradle.kts` adds `kotlinx-coroutines-core` + `kotlinx-coroutines-test` to `testImplementation` because `:kformik` declares coroutines via `implementation` (not `api`) and the end-to-end test drives suspend methods directly.
- **`onReset_callback_firesOnResetForm` in `ComposeFormikLifecycleTest` (`:kformik-compose` jvmTest)** pins the `rememberFormik(onReset = ...)` wiring through the Compose adapter (`RememberFormik.kt`). Previously only the controller-level `ResetTest` exercised `onReset`; the Compose-adapter wire-up (which threads the callback through `rememberUpdatedState`) could have silently broken without the regression test.
- **Production-readiness sweep: +229 tests across the four published modules** (`:kformik` +35, `:kformik-compose` +13, `:kformik-forms` +123, `:kformik-ksp` +6, plus a Compose-UI test harness wiring in `:kformik-forms/build.gradle.kts`). Project-wide totals: 840 tests (jvm + ios simulator), up from 611. Adds `EdgeCasesTest` (`:kformik` core), `ApiContractTest` (`:kformik` core), `RememberFormikRegressionTest` + `ComposeFormikLifecycleTest` (`:kformik-compose`), 7 per-renderer suites + `KformikFormUiTest` + `EdgeCasesUiTest` + `StaleStateRegressionTest` + `TestHelpers`/`TestHelpersSmokeTest` (`:kformik-forms`), `ProcessorErrorMessagesTest` (`:kformik-ksp`), `ConcurrencyStressTest` appends. Every documented public behavior has at least one observable-state test; the four load-bearing v1.9.2 stale-state regressions are pinned (`fieldState`, `valuesUpdater`, `NumberRenderer.displayBuffer`, `SelectRenderer.expanded`, `DateRenderer.LaunchedEffect(form)`). New `docs/TEST_COVERAGE.md` maps every public API symbol to its owning test file plus the per-platform coverage table.

### Docs

- **KDoc tightenings for three surprising-but-correct-by-design contracts** (issue #3 D1 / D2 / D3). No public API change; `apiCheck` clean; Dokka builds.
  - **D1 — `FormikActions.setFieldValue` null prunes the key.** A runtime `setFieldValue(name, null)` PRUNES the leaf from the backing `Map` per the `MapValuesUpdater` contract — `state.values.containsKey(name) == false` afterwards — while an initial-values `null` IS preserved with the key intact. `EdgeCasesTest.nullValue_isReadable_andDistinctFromMissing` already pins the behavior; the new "null semantics" KDoc note in `FormikActions.kt` makes the contract discoverable from the public surface. A cross-reference in `Field.initialValue` (`:kformik-forms`) explains how the "explicit `null`" Select / Radio escape hatch (v1.9.0 `FieldDefaultValue` sentinel) differs from a runtime `null` write.
  - **D2 — `ComposeFormik.controller.state.value` does not subscribe Composables.** Added a paragraph to the `ComposeFormik` class KDoc telling consumers to prefer the `@Composable`-getter forms (`state`, `dirty`, `isValid`, `fieldState`) inside a Composable body, and reserve the underlying `StateFlow.value` accessors for non-Composable contexts. Surfaced by `KformikFormUiTest.footerSlot_canRead_formState`, where reading `form.controller.state.value` inside the slot lambda produced a snapshot that never recomposed.
  - **D3 — `FormikActions.setSubmitting` is a display flag only.** Added a "this does NOT gate `submit()`" note to `setSubmitting`'s KDoc. The single-flight gate is the internal `submitMutex` (v1.8.0); `setSubmitting` drives display state only. Tested contract in `ApiContractTest.setSubmitting_doesNotBlockSubmit_gateIsStructural`; the KDoc makes the asymmetry explicit so consumers don't expect `setSubmitting(true)` to prevent re-submits.

### Coordinates

```
io.github.apdelrahman1911:kformik:1.9.3
io.github.apdelrahman1911:kformik-compose:1.9.3
io.github.apdelrahman1911:kformik-forms:1.9.3
io.github.apdelrahman1911:kformik-ksp:1.9.3
```
